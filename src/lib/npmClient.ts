...
export class NpmClient {
  ...
  async publish(packagePath: string, options: { tag?: string; access?: string } = {}) {
  ...
  async getDistTags(pkg: string) {
    const response = await fetch(
      `${this.registry}/${encodeURIComponent(pkg)}`,
    );
  ...
  async getOrgInfo(orgname: string) {
    const token = await this.getToken();
    const response = await fetch(`${this.api}/org/${encodeURIComponent(orgname)}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
