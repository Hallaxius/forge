...
export default function register(program: Command): void {
  const distTagProgram = program.command("dist-tag").description("Manage distribution tags");
  ...
  distTagProgram
    ...
    .action(async (pkg: string, tag: string) => {
      try {
        const npmClient = new NpmClient();
        await withSpinner(
          `Removing dist-tag ${tag} from ${pkg}...`,
  ...
  distTagProgram
    ...
    .action(async (pkg: string) => {
      try {
        const npmClient = new NpmClient();
        const tags = await withSpinner(
          `Fetching dist-tags for ${pkg}...`,
        ...
        const rows = Object.entries(tags).map(([tag, version]) => [tag, version as string]);
